...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    foreach ($sheet->getRowIterator() as $row) {
        ...
        // Only process rows where C column == "Faktur"
        if ($type !== 'Faktur' || empty($idValue)) {
            continue;
        }
    
        // Extract Year and Month from Column B (ID)
        if (preg_match('/PU(\d{4})(\d{2})\d+/', $idValue, $matches)) {
            $year = $matches[1];  // YYYY
            $monthNumber = $matches[2]; // MM
    ...
    $zip = new ZipArchive();
    $zipFileName = $outputDir . "Faktur_Files.zip"; // ZIP file path
    ...
    if ($zip->open($zipFileName, ZipArchive::CREATE | ZipArchive::OVERWRITE) === TRUE) {
        ...
        foreach ($dataByMonthYear as $year => $months) {
            foreach ($months as $monthName => $rows) {
                ...
                // Set headers
                // $newSheet->setCellValue('A1', 'Date');
                // $newSheet->setCellValue('B1', 'Column B');
                // $newSheet->setCellValue('C1', 'Column E (Cleaned)');
                ...
                // Save the new file
                $fileName = "Faktur_{$monthName}_{$year}.xlsx";
